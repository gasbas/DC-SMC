...
def construct_output_df(posterior_data, target_posterior = 'theta') : 
    ...
    for level, data in posterior_data.items() : 
        for node_name, values in data.items() : 
            ...
            var = np.mean(curr_data['variance']) if target_posterior == 'theta' else curr_data['std']
            ...
            if target_posterior == 'theta' : 
                curr = pd.DataFrame({
                    ...
                    'samples' : curr_data['samples'],
                    'mean_samples' : curr_data['mean'],
                    'var_samples' : curr_data['variance']},
...
def density_plot(results, save_path = None, max_sample = 1000, legend_names = None) : 
    ...
    if save_path is not None : 
        ax.savefig(save_path + 'density_plot.png', dpi = 300)
    plt.show()
    plt.close()
...
def trace_plot(result, save_path = None, max_sample = 1000) : 
    ...
    if max_sample > len(result.particles) : 
        max_sample = result.particles
    samples_LL = []
    for j in range(max_sample) : 
        samples_LL.append(result.sample_logdensity())
        
    plt.Figure(figsize=(8,6))
    plt.plot(samples_LL, c = 'black', lw = 0.7)
...
def plot_posterior(posterior_data, target_posterior = 'theta', target_func = 'mean', max_cols = 5, 
    ...
    ax[-1][0].set_xlabel(list(tmp[idx][0].keys())[0])
    for idx in range(len(tmp)) :
        
        ax[idx][0].set_ylabel(row_names[idx])
        ...
        for level in range(0,n_cols-1) : 
            ...
            name = list(tmp[idx][1].keys())[level]
            ax[-1][level+1].set_xlabel(name)
            
            sns.kdeplot(tmp[idx][1][name][target_posterior][target_func], color = colors[idx], lw = 1.5,
...
def ess_plot(ess, threshold, save_path = None) : 
    ...
    plt.hlines(xmin = 0, xmax = len(ess), y = threshold, linestyle = '--', color = 'grey', alpha = 0.7, lw = 1.2)
    plt.grid(visible = True)
    if save_path is not None : 
        plt.savefig(save_path + 'trace_plot.png', dpi = 300)
    plt.show()
